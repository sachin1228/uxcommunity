...
const PNG_BASE = "https://fonts.gstatic.com/s/e/notoemoji/latest";
        ...
        for (const icon of data.icons) {
          ...
          if (codepoint) {
            ...
            emojis.push({
              ...
              pngUrl: `${PNG_BASE}/${codepoint}/png/64`,
            });
...
/**
 * Get the PNG URL for an emoji character (for fallback/preview)
 */
export async function getEmojiPngUrl(emoji: string): Promise<string | null> {
  const codepoint = emojiToCodepoint(emoji);
  if (!codepoint) return null;
  
  const emojiData = await getEmojiByCodepoint(codepoint);
  return emojiData?.pngUrl ?? null;
}
