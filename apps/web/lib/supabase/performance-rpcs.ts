...
type PerformanceRpcMap = {
  get_community_message_page: {
    ...
    returns: Array<{ id: string; content: string | null; created_at: string; user_id: string; reply_to_id: string | null; image_url: string | null; deleted_at: string | null; edited_at: string | null; users: Json; reactions: Json; reply_to: Json }>;
  };
  ...
  get_showcase_list_page: {
    ...
    returns: Array<AggregateBase & { community_id: string; user_id: string; title: string; description: string; image_url: string; category: string; created_at: string; updated_at: string; author: Json; like_count: number; comment_count: number; user_liked: boolean; user_saved: boolean }>;
  };
