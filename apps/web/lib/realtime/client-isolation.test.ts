 ...
 *   carrying every user-scoped room (notifications:*, profile:*) as subscribe
 *   frames.
...
// ══════════════════════════════════════════════════════════════════════════
// TEST 1: the user-scoped socket is scoped to the individual client instance
// ══════════════════════════════════════════════════════════════════════════
...
test("every user-scoped room multiplexes over ONE socket to the user's DO", () => {
  ...
  assert.deepStrictEqual(getConnectionKeys(clientA), ["user:user-a"],
    "user-scoped rooms must share a single socket — one socket per room would miss the DO the server publishes to");
...
test("community connections are keyed by room name, not the user socket", () => {
