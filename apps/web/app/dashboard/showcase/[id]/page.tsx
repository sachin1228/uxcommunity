...
export default async function PublicShowcaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: row } = await db.from("community_showcase_posts").select("*").eq("id", postId).is("community_id", null).eq("is_public", true).maybeSingle();
  ...
  const post = { ...row, community_id: "public", author: { name: author?.name ?? "Community member", avatar_url: profile?.avatar_url ?? null }, like_count: likes?.length ?? 0, comment_count: enriched.length, user_liked: Boolean(myLike), user_saved: Boolean(mySave) } as ShowcasePost;
  return (
    <div className="flex h-full items-start">
      <div className="min-w-0 flex-1 border-r border-border">
        <ShowcaseDetailClient initialPost={post} initialComments={comments} currentUserId={userId} communityId="public" backHref="/dashboard" />
      </div>
