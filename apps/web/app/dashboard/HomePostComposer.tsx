...
export function HomePostComposer({ name, avatarUrl, onCreated }: HomePostComposerProps) {
  ...
  return (
    <>
      <section className="mx-8 w-[calc(100%-3rem)] my-2 max-w-4xl overflow-hidden rounded-xl shadow-sm">
          ...
          <div className="grid min-w-0 grid-cols-3 justify-items-center gap-0.5">
            {postTypes.map(({ type, label, icon: Icon, color }) => (
              <button
                ...
                className="flex w-fit min-w-0 items-center justify-self-center gap-1 rounded-lg px-2 py-2 font-body text-sm font-medium text-foreground-muted transition-colors hover:bg-surface-raised hover:text-foreground sm:gap-1.5"
              >
