...
export async function DELETE(
  ...
  const urls = await collectMasterMediaUrls(db, "city", "cities", id);
  ...
  try {
    const cleanup = await cleanupMasterDataMedia(db, "city", "cities", id, urls);
    if (cleanup.failed.length > 0) {
      console.error("[admin/cities] R2 cleanup failures:", cleanup.failed);
    }
    ...
    console.error("[admin/cities] R2 cleanup error:", cleanupError);
