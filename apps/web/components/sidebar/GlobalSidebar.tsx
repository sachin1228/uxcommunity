...
import { Home, MessageSquare, Plus, Swords, Users } from "lucide-react";
...
export function GlobalSidebar({ userId, mobile = false }: Props) {
  ...
  const duelActive = isMatch("/dashboard/design-duel", pathname);
        ...
        <ul className="space-y-1">
          ...
          <li>
            ...
            >
              ...
              <span className="flex-1 truncate">Design Duel</span>
              {duelActive && (
                <span className="h-1.5 w-1.5 rounded-full bg-accent shrink-0" />
              )}
            </Link>
