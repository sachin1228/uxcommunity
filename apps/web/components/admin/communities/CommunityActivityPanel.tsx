...
const ROLE_CHIP: Record<CommunityActivityEntry["actor_role"], string> = {
  platform: "bg-popover text-muted-foreground border-border",
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="px-5 py-3 border-b border-border">
        <h2 className="font-body text-xs font-semibold text-foreground flex items-center gap-1.5">
          <Activity strokeWidth={2.5} size={13} className="text-primary" />
            ...
            <Button variant="ghost"
              ...
              className={`rounded-full px-2.5 py-1 font-body text-[11px] transition-colors border ${
                filter === actor.id
                  ...
                  : "border-border text-muted-foreground hover:text-foreground hover:bg-popover"
              }`}
            >
              {actor.name}
            </Button>
      ...
      {loading ? (
        ...
        <div className="px-5 py-6 flex items-center gap-3">
          <ShieldCheck strokeWidth={2.5} size={16} className="text-muted-foreground/50 shrink-0" />
              ...
              <div key={entry.id} className="flex items-start gap-3 px-5 py-3">
                <span
                  className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border font-body text-[9px] font-bold ${
                    isPlatform ? "border-border bg-popover text-muted-foreground" : ROLE_CHIP[entry.actor_role]
                  }`}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-body text-[10px] text-muted-foreground/70 mt-0.5">
                    ...
                    {!isPlatform && entry.actor_role === "owner" && (
                      <span className="ml-1.5 uppercase tracking-wider text-[9px] text-primary/70">owner</span>
                    )}
                  </p>
                </div>
              </div>
