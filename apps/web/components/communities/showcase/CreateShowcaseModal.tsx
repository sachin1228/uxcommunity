  ...
  return <ModalPortal><div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="showcase-form-title" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form onSubmit={submit} className="modal-panel flex max-h-[min(800px,calc(100vh-2rem))] w-full max-w-2xl flex-col overflow-hidden"><div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5 pt-5">
      ...
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        ...
        <div className="md:col-span-2"><span className="mb-2 block font-body text-xs text-muted-foreground">Preview image *</span><button type="button" onClick={() => fileRef.current?.click()} className="flex min-h-48 w-full items-center justify-center overflow-hidden rounded-2xl border border-dashed border-border bg-popover text-muted-foreground hover:border-primary/60">{preview ? <img src={preview} alt="Work preview" className="max-h-80 w-full object-cover"/> : <span className="flex flex-col items-center gap-2 font-body text-sm"><ImagePlus strokeWidth={2.5} size={24}/>Choose a cover image</span>}</button><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => { const next = event.target.files?.[0] ?? null; setFile(next); if (next) setPreview(URL.createObjectURL(next)); }}/></div>
        <label><span className="mb-1.5 block font-body text-xs text-muted-foreground">Category</span><select className={field} value={category} onChange={(event) => setCategory(event.target.value as ShowcaseCategory)}>{SHOWCASE_CATEGORIES.filter((item) => item.value !== "all").map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
      </div>
      ...
      {error && <p className="mt-4 font-body text-sm text-red-400">{error}</p>}</div><div className="flex shrink-0 items-center justify-end gap-3 border-t border-border p-3"><button type="button" onClick={onClose} className="modal-btn modal-btn-secondary">Cancel</button><button disabled={saving} className="modal-btn modal-btn-primary">{saving && <Spinner size={15} className="text-white" />}{saving ? "Saving…" : editing ? "Save changes" : "Share work"}</button></div>
