 ...
 *
 * The bug these lock down: the server published `notifications:${userId}` into
 * the COMMUNITY_DO namespace, while every browser's user-scoped socket lives in
 * that user's USER_DO instance. The event therefore landed in a Durable Object
 * with no sockets attached — `/publish` still answered "ok", so nothing looked
 * broken and every client silently fell back to its periodic refetch.
 *
 * These tests connect a real WebSocket the way the app does: `room=user:${id}`
 * (one socket per user), with the logical user-scoped rooms multiplexed over it
 * as subscribe frames, and assert the event actually arrives.
 */
