-- Fetch a bounded page of community messages and all display enrichment in one
-- database round trip. Apply this migration manually before deploying the route
-- that calls get_community_message_page.
...
as $$
  with message_page as materialized (
    ...
    where message.community_id = p_community_id
      and message.created_at >= p_history_start
      and (p_before is null or message.created_at < p_before)
      and (p_after is null or message.created_at > p_after)
  ...
  -- Keep the caller parameter in the signature so future caller-specific
  -- enrichment can be added without changing the route contract.
  where p_user_id is not null
  order by message.created_at desc;
$$;

revoke all on function public.get_community_message_page(uuid, uuid, timestamptz, timestamptz, timestamptz, integer) from public;
revoke all on function public.get_community_message_page(uuid, uuid, timestamptz, timestamptz, timestamptz, integer) from anon;
revoke all on function public.get_community_message_page(uuid, uuid, timestamptz, timestamptz, timestamptz, integer) from authenticated;
