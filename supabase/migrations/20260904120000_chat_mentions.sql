...
-- Chat mentions
--
--  1. community_messages.mentions — JSONB array of
--     { user_id, name } for the members @mentioned in a message.
--     The name is snapshotted at send time so mentions still
--     render (and stay correct) after a member renames.
--  2. get_community_message_page returns `mentions` so message
--     pages, bootstrap snapshots and poll catch-up can render
--     mention chips for history without extra round trips.
--  3. notifications accepts type 'chat_mention' with entity
--     type 'message' so a mentioned member gets a bell row that
--     deep-links back to the exact message.
-- ============================================================

alter table public.community_messages
  add column if not exists mentions jsonb not null default '[]'::jsonb;

-- The message page RPC has a fixed table return type, so it must be
-- recreated when the new field is added to its result.
...
-- ─── notifications: allow chat mentions ─────────────────────────────────────
...
alter table public.notifications
  add constraint notifications_entity_type_check check (
    entity_type in ('community', 'thread', 'resource', 'event', 'message')
  );
