-- Remove the showcase "stage" field ("Where is this in the process?").
--
-- The composer no longer asks for it, so the column and the RPC that returned
-- it go away together. This product is still in beta, so existing stage values
-- are simply dropped.

alter table public.community_showcase_posts
  drop column if exists stage;

-- The function's return type changes, so it has to be dropped before it can be
-- recreated without `stage`.
