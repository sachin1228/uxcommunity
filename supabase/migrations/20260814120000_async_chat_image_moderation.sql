alter table public.community_messages
  ...
  add column if not exists image_moderation_error text;

alter table public.community_messages
  drop constraint if exists community_messages_image_status_check;

alter table public.community_messages
  add constraint community_messages_image_status_check
  check (image_status is null or image_status in ('pending', 'approved', 'rejected', 'review_required'));

create index if not exists community_messages_pending_image_moderation_idx
...
as $$
  with message_page as materialized (
    (
      select message.id, message.content, message.created_at, message.user_id,
        message.reply_to_id, message.image_url, message.image_status, message.deleted_at
  ...
  select
    ...
    message.reply_to_id,
    case when message.image_status in ('pending', 'rejected', 'review_required') then null else message.image_url end,
