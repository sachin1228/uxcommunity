-- Scope get_community_message_page to an optional channel so the chat
-- view can page through a single subchannel's messages.
-- Apply after 20260820000000_community_channels.sql (needs channel_id).
-- p_channel_id is the LAST parameter so existing 6-argument callers
-- keep binding to the same positions and simply default to the
-- community-wide (general) read.
  ...
  with message_page as materialized (
    (
      ...
      where p_after is not null
        ...
        and (p_channel_id is null or message.channel_id = p_channel_id)
...
revoke all on function public.get_community_message_page(uuid, uuid, timestamptz, timestamptz, timestamptz, integer, uuid) from public, anon, authenticated;
