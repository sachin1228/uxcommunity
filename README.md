...
### Demo competition data (optional)

A fresh database has five competition cycles and no entries, so the competitions pages open on an 
empty state. To fill the started weeks with realistic entries, votes and comments — attributed to 
real members, with generated artwork committed under `apps/web/public/competition-demo/` — run the 
opt-in seed:

```bash
npm run seed:competitions-demo   # needs DATABASE_URL in the environment
```

It is deliberately hard to fire by accident: the file lives outside `supabase/migrations/` (so
`supabase db push` can never run it) and does nothing unless `app.competition_demo_seed=on` is set,
which only the npm script does. It writes only into cycles that have already started, and skips any 
cycle that already has demo entries, so re-running it is a no-op. Every seeded entry carries a 
`demo` tag, which shows on the entry detail page.
...
Entries cascade their own votes and bookmarks. The artwork is generated by 
`npm run art:competitions-demo` (`scripts/generate-competition-demo-art.mjs`) and is committed, so 
the seed works without regenerating it.
